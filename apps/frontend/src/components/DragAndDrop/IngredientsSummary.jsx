...
export function IngredientsSummary({ totals }) {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mt-6">
      <h3 className="text-xl font-bold text-slate-gray mb-4">
        Totales de Ingredientes
      </h3>
          ...
          <tbody className="text-sm">
            {totals.map((total, index) => (
              <tr key={index} className="border-b border-gray-50 last:border-0">
                <td className="py-3 text-slate-gray font-medium">
                  {total.name}
                </td>
                <td className="py-3 text-slate-gray text-right">
                  {total.totalQuantity.toLocaleString("es-VE", {
                    maximumFractionDigits: 2,
                  })}{" "}
                  {total.measurementUnit}
                </td>
              </tr>
            ))}
          </tbody>
