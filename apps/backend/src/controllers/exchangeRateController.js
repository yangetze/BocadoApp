...
// --- Module-Level Cache ---
// We cache the base currencies once per module lifetime to avoid redundant DB queries
let cachedUsdCurrency = null;
let cachedVesCurrency = null;

// Exported for testing purposes to reset the cache
export const clearCurrencyCache = () => {
  cachedUsdCurrency = null;
  cachedVesCurrency = null;
};

// Helper function to efficiently ensure default currencies exist using the cache
const ensureDefaultCurrencies = async () => {
  if (!cachedUsdCurrency) {
    cachedUsdCurrency = await prisma.currency.findUnique({ where: { code: 'USD' } });
    if (!cachedUsdCurrency) {
      cachedUsdCurrency = await prisma.currency.create({
        data: { code: 'USD', symbol: '$', isBase: true }
      });
    }
  }

  if (!cachedVesCurrency) {
    cachedVesCurrency = await prisma.currency.findUnique({ where: { code: 'VES' } });
    if (!cachedVesCurrency) {
      cachedVesCurrency = await prisma.currency.create({
        data: { code: 'VES', symbol: 'Bs', isBase: false }
      });
    }
  }

  return { usd: cachedUsdCurrency, ves: cachedVesCurrency };
};
...
export const fetchAndStoreApiRate = async (req, res) => {
  try {
    ...
    // Ensure default currencies exist and use the cached VES currency
    const { ves: targetCurrency } = await ensureDefaultCurrencies();
