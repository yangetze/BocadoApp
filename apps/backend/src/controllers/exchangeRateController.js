...
// Module-level cache to prevent redundant database queries for default currencies
let defaultCurrenciesInitialized = false;

const ensureDefaultCurrencies = async () => {
  if (defaultCurrenciesInitialized) return;
  if (isTestMode()) {
    defaultCurrenciesInitialized = true;
    return;
  }
...
export const fetchAndStoreApiRate = async (req, res) => {
  try {
    ...
    // 2. Ensure default currencies exist and find the VES Currency ID
    await ensureDefaultCurrencies();
