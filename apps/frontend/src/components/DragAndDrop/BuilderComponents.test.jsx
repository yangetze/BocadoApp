...
describe('Builder Extracted Components', () => {
  describe('BuilderHeader', () => {
    it('renders correct title and description for superRecipe mode', () => {
      ...
      expect(screen.getByText('Construye tu producto final apilando recetas base.')).toBeInTheDocument();
    });
  ...
  describe('MarginRecommendationCard', () => {
    it('does not render if suggestedMargin is null', () => {
      const { container } = render(<MarginRecommendationCard suggestedMargin={null} />);
      expect(container).toBeEmptyDOMElement();
    });

    it('renders the suggested margin', () => {
      render(<MarginRecommendationCard suggestedMargin={40} />);
      expect(screen.getByText('Recomendador de Margen (IA)')).toBeInTheDocument();
  ...
  describe('BaseRecipeMetadataForm', () => {
    it('renders inputs and total cost', () => {
      const mockMetadata = { name: 'Test Recipe', baseYield: 500, yieldUnit: 'g' };
    ...
    it('calls setMetadata on input change', () => {
      ...
      fireEvent.change(screen.getByPlaceholderText('Ej. Ganache de Chocolate'), {
        target: { value: 'New Name' }
      });

      expect(mockSetMetadata).toHaveBeenCalledWith({ ...mockMetadata, name: 'New Name' });
    });
  });
});
